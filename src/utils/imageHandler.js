...
export async function generateBase64Logo(logoPath, height = 30) {
    try {
        ...
        const buffer = await sharp(logoPath).resize({ height }).png({ quality: 80 }).toBuffer();

        const base64 = buffer.toString('base64');
        logger.info('📐 Resizing and encoding logo');
        return `data:image/png;base64,${base64}`;
    } catch (err) {
        logger.error(`Error processing image at path: ${logoPath} - ${err.message}`);
        throw err;
    }
}
...
export function embedLocalImagesInMarkdown(markdown, sourceDir) {
    ...
    // Matches ![alt](path) and ![alt](path "title") — captures path and optional title separately
    return markdown.replace(
        /!\[([^\]]*)\]\(([^)"'\s]+)((?:\s+(?:"[^"]*"|'[^']*'))*)?\)/g,
        (match, alt, imgPath, titlePart) => {
            const resolvedPath = path.resolve(sourceDir, imgPath);

            if (!fs.existsSync(resolvedPath)) {
                logger.warn(`Image not found: ${imgPath}. Path resolved to: ${resolvedPath}`);
                return match;
            }
            ...
            return `![${alt}](data:${mime};base64,${base64}${titlePart ?? ''})`;
        },
    );
}
