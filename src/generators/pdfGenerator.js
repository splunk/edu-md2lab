...
function getChromeExecutablePathFromEnv() {
    return (
...
function getDetectedSystemChromePath() {
...
function getPuppeteerLaunchOptions() {
    ...
    const executablePathFromEnv = getChromeExecutablePathFromEnv();
    const detectedSystemPath = executablePathFromEnv ? '' : getDetectedSystemChromePath();

    const executablePath = executablePathFromEnv || detectedSystemPath;
...
export async function addHeadersAndFootersToPdfBuffer(
    ...
    pages.forEach((page, index) => {
        ...
        const availableWidth =
            width - (marginLeft + copyrightWidth + padding) - (pageNumWidth + marginLeft + padding);
            ...
            for (const word of words) {
                ...
                if (testWidth <= maxWidth) {
                    ...
                    if (currentLine) {
                        ...
                        while (
                            font.widthOfTextAtSize(truncatedWord + '...', fontSize) > maxWidth &&
        ...
        if (titleLines.length > maxLines) {
            ...
            if (ellipsisWidth <= availableWidth) {
                ...
                while (
                    helveticaFont.widthOfTextAtSize(truncatedLine + '...', 9) > availableWidth &&
...
export async function generatePdf(sourceDir, metadata, datestamp, options = {}) {
    ...
    if (files.length === 0) {
        ...
        logger.error('Please make sure the directory contains one or more .md files.');
        process.exit(1);
    }
    ...
    const renderVariants = [{ includeAnswers: false, suffix: '', label: 'without answers' }];
